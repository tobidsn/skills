...
// ✅ Prefer: Thin controller with dedicated classes + ApiResponse + Resource
...
Route::get('/tasks/{task}', fn(Task $task) => ['data' => $task]);
Route::post('/tasks', fn() => response()->json(['task' => $created]));

// ✅ Prefer: ApiResponse + API Resources (see .cursor/rules/api-development.mdc)
Route::get('/tasks', fn() => ApiResponse::paginate(TaskResource::collection($paginator), [], 'OK'));
Route::get('/tasks/{task}', fn(Task $task) => ApiResponse::json(TaskResource::make($task)));
Route::post('/tasks', fn() => ApiResponse::created(TaskResource::make($created), 'Created'));
```
