...
| `pdf-to-html` | Rebuild a PDF as one self-contained HTML file that keeps the original font, layout, and page breaks. Measures the source at 300 dpi (glyph coordinates, box rules, colours) instead of eyeballing it, builds against those numbers, then renders the result back to PDF and diffs it rule by rule and character by character. Ships `probe` / `measure` / `inline` / `verify` scripts plus a page template |
