...
| `security-audit` | audit → plan → build, each phase gated. Audit = one ranked `path:line` table (≤10 rows) + `Not scanned:` line; scans every detected ecosystem (composer + npm coexist in a Laravel app), `govulncheck` via `go run` without installing. Plan lands at `<audited project>/docs/security-audit/<date>-<slug>-plan.md`, CRIT/HIGH only, findings table embedded. `audit auto` skips the plan confirmation; the build gate never opens without explicit approval. Language patterns in `references/{php,typescript,go}.md` + `plan-template.md`, all loaded on demand |
