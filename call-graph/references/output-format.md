...
- **`[proposed]` changes what `src:` may contain, and often empties it.** A proposal drawn from nothing has nothing to cite: drop the block and say in one line that the graph reflects described intent rather than a verified source. A proposal drawn *into an existing codebase* is the exception — nodes that already exist keep their real `path:line`, `[new]` nodes never appear in the block at all, and a closing `[new] nodes — not yet implemented` line says why the two counts differ. What is never allowed is a line number for something that doesn't exist yet. See `design-graphs.md`.
